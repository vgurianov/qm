...
class Composite(Component): # <<Category>>
    ...
    def add(self, c):
        self.head.component = c
    def remove(self):
        c = self.tail.component
...
class OneClass(Composite):  # <<System>>, instead of Mix
    ...
    def move_to_x1(self, p):  
        """ Concept = particle displacement A """
        self.head.right.top = p
        return self.head.right.top
    def move_to_x2(self, p):  
        """ Concept = particle displacement B """
        self.head.right.bottom = p
        return self.head.right.bottom
    ...
    def Run(self): # <<Exist>>
        ...
        if r<p:
            ...
            if rr == 0:
                self.way = A.way_a  # self.way is a control variable
                self.tail = self.move_to_x1(self.head)  # Path A
            else:
                self.way = B.way_b  # self.way is a control variable
                self.tail = self.move_to_x2(self.head)  # Path B
        else:
            self.way = None
